...
async function eliminarAlumno(formData) {
    ...
    const consulta = `http://localhost:4000/alumnos/${id}`
...
async function insertarAlumno(formData) {
    ...
    await fetch('http://localhost:4000/alumnos', {
        method: 'POST',
                        ...
                        data.map((alumno) => (
                            <div key={alumno.id} className="p-4 bg-white shadow-sm rounded-md flex justify-between items-center">
                                ...
                                <div className="flex space-x-2">
                                    <Link href={`/alumnos-api/${alumno.id}`} className="text-blue-500 hover:underline">Ver</Link>
                                    <Link href={`/alumnos-api/${alumno.id}/modificar`} className="text-yellow-500 hover:underline">Modificar</Link>
